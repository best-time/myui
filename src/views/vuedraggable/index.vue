  ...
  <div class="row">
    <div style="margin-bottom: 100px; padding: 40px">
      <div :key="index" v-for="(item, index) in mapList">
        <p>{{ item.title }}</p>
        <draggable class="list-group" :list="item.list" :group="'art' + index" itemKey="id" @start="start" @end="end">
          <template #item="{ element, index }">
            <div class="list-group-item" :data-prop="toStr({ element, type: item.componentType })">
              {{ element.name }} --------- {{ index }}
            </div>
          </template>
        </draggable>
