...
const usedialogStore = defineStore('dialogStore', () => {
  ...
  /**在第一个弹框中再次新增一个同级的页面 */
  const addDialog = (childrenOption) => {
    if (!dialogOption.dialogTitle) {
      return console.error('请确保是否含有初始弹框容器,再添加新的弹框页面')
    }

    // 此处使用名称作为唯一标识 若二次弹框的名称一致,可自定义id 属性区分
    const obj = dialogOption.children.find((item) => item.topTagName === childrenOption.topTagName)
    activeComponent.value = dialogOption.children.length
    if (obj) {
      //如果已经存在了某个页面 但是没有关闭  再次点击则不新增新页面 而是回到旧页面
      activeComponent.value = dialogOption.children.findIndex((item) => item.topTagName === childrenOption.topTagName)
      return
    }
