...
export function createRenderHook<T = void>(config?: { fragment?: boolean; single?: boolean; effects?: iEffects }) {
  ...
  const exec: iExecFunc = (execData?: Partial<iHookData>) => {
    ...
    if (config?.single) {
      ...
      /*渲染多个renderMeta*/
      return execData!.renderMetas.map((renderMeta) =>
        config?.fragment !== false
          ? h(Fragment as any, { key: renderMeta.key }, renderMeta.render(execData!.renderData!) as any)
          : renderMeta.render(execData!.renderData!)
      )
    }
  }
...
export type iHookDataType<T> = [T] extends [void]
  ? { renderMetas: iRenderMeta<T>[]; renderData?: T }
  : { renderMetas: iRenderMeta<T>[]; renderData: T }

export type iExecFuncType<T, iHookData> = [T] extends [void]
  ? (execData?: Partial<iHookData>) => RenderNode
  : (execData: Partial<iHookData>) => RenderNode
