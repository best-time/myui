...
import { _globalWatch, _sourceWatch } from "../effect/_watch";
import { ComputedRef, getCurrentInstance, markRaw, onBeforeUnmount } from "vue";
 ...
 */
const _cacheComputed = <T>(getter: () => T, shouldUpdate?: (newValue: T, oldValue: T) => boolean, effects?: iEffects): ComputedRef<T> => {
  ...
  const { effects: _effects } = createEffects();
  !!effects ? effects.push(_effects.clear) : onBeforeUnmount(_effects.clear);
...
export const cacheComputed: typeof _cacheComputed = (getter, shouldUpdate) => {
  const ctxEffects = useInstanceEffects();
  if (!ctxEffects) {
    logger.setupError("cacheComputed");
    return _cacheComputed(getter, shouldUpdate);
  } else {
    return _cacheComputed(getter, shouldUpdate, ctxEffects);
  }
};

export const globalCacheComputed = _cacheComputed;
