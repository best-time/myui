...
export default {
  ...
  props: {
    position: {
      validator(value) {
        return oneOf(["bottom", "center", "left", "right"]);
      },
      default: "bottom"
    },
    height: {
  ...
  watch: {
    value(val) {
      if (isIOS) {
        ...
        const topHeight =
          !!this.$slots.top && this.position != "center"
            ? $refs.top.offsetHeight
            : 0;
        const bottomHeight =
          !!this.$slots.bottom && this.position != "center"
            ? $refs.bottom.offsetHeight
            : 0;
        const contentHeight =
          topHeight + $refs.content.offsetHeight + bottomHeight;
  ...
  methods: {
    ...
    classes() {
      return (
        (this.position == "center" ? "m-popup-center " : "m-popup ") +
