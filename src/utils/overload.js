 ...
 * 内部类型函数标志
 */
export const INNER_TYPE_FN = "##INNER_TYPE_FN##";
...
function createOverload() {
  const TYPES = []; window.TYPES = TYPES
  const FNS = [];window.FNS = FNS
  ...
  function overload(...params) {
    ...
    loop: for (let i = 0; i < TYPES.length; i++) {
      ...
      // 类型长度不一致, 且不是...
      const typeLengthNotEqual = options.addTypesLength !== paramsLength && !options.rest

      // 调用不传参数, 但定义有参数,且不是...
      const noParamsNotEqual = noParam && typesLength && types[0] !== REST_STR

      if (typeLengthNotEqual || noParamsNotEqual) {
        continue;
      }

      for (let j = 0; j < paramsLength; j++) {
        const type = types[j] || types[typesLength - 1];
        let matched = matchType(params[j], type)
        if (!matched) {
          try {
            const convert = type?.[TYPE_CONVERT_STR]?.(params[j]);
            matched = matchType(convert, type)
            if (matched) {
              params[j] = convert;
              continue;
            }
...
const ins = createOverload()
export default ins.add([], function () {
