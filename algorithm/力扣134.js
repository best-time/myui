...
function get(gasList, costList) {
    const len = gasList.length
    let start = 0
    let curSum = 0
    let totalSum = 0
    for(let i = 0; i < len; i++) {
        curSum += gasList[i] - costList[i]
        totalSum += gasList[i] - costList[i]
        if(curSum < 0) {
            curSum = 0
            start = i + 1
        }
...
console.log(get([1,2,3,4,5], [3,4,5,1,2]))
