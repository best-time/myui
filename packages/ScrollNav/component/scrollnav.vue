<template>
  <div class="scrollnav">
    ...
    >
      <div
        class="scrollnav-unfold"
        :class="toggle ? 'scrollnav-unfold-active' : ''"
      >
        <div
          class="scrollnav-unfold-header"
      ...
      <div
        class="scrollnav-toggle"
        :class="toggle ? 'scrollnav-toggle-active' : ''"
        @click="toggle = !toggle"
      ></div>
    </div>
  ...
  methods: {
    ...
    getPanels() {
      return this.$children.filter(
        item => item.$options.name === "scrollnav-panel"
      );
    },
    scrollHandler() {
      ...
      if (
        scrollBox.scrollTop >=
        panels[0].$el.offsetHeight * panelsLength - scrollBox.offsetHeight
      ) {
      ...
      panels.forEach((panel, index) => {
        if (
          panel.$el.getBoundingClientRect().top <=
          scrollBox.offsetHeight / 2 + this.contentOffsetTop
        ) {
          this.activeIndex = index;
        }
      });
    ...
    scrollNav(_uid, animate = true) {
      ...
      this.navList.every((item, index) => {
        if (item._uid === _uid) {
          ...
          const scrollOffset =
            navitem.offsetLeft - navWidth + navitem.offsetWidth / 2;
    ...
    scrollContent(index, animate = true) {
      ...
      const speed =
        animate &&
        ((window.navigator && window.navigator.userAgent) || "").indexOf(
          "MicroMessenger"
        ) < 0
          ? 500
          : 0;
      ...
      scrollTop(
        ...
        panel.offsetTop - this.$refs.navbox.offsetHeight,
        speed,
        () => {
