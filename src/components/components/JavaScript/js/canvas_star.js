;(function () {
  ...
  function drawSingleStar(el) {
    ...
    for (let i = 0; i < 5; i++) {
      context.lineTo(
        Math.cos(((18 + i * 72) / 180) * Math.PI) * radius + radius,
        -Math.sin(((18 + i * 72) / 180) * Math.PI) * radius + radius
      )
      context.lineTo(
        Math.cos(((54 + i * 72) / 180) * Math.PI) * angle + radius,
        -Math.sin(((54 + i * 72) / 180) * Math.PI) * angle + radius
      )
    }
    context.closePath()
  ...
  function drawStarList(option) {
    ...
    /** 半径 */
    const radius = option.starWidth
      ? option.starWidth / 2
      : (canvas.width - (starList.length - 1) * interval) / starList.length / 2
    /** 每个角的厚度 */
    const angle = radius / (150 / 68)
    ...
    function drawStarSing(index) {
      ...
      for (let i = 0; i < 5; i++) {
        context.lineTo(
          Math.cos(((18 + i * 72) / 180) * Math.PI) * radius + radius + x,
          -Math.sin(((18 + i * 72) / 180) * Math.PI) * radius + radius
        )
        context.lineTo(
          Math.cos(((54 + i * 72) / 180) * Math.PI) * angle + radius + x,
          -Math.sin(((54 + i * 72) / 180) * Math.PI) * angle + radius
        )
      }
      context.closePath()
