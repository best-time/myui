<template>
  <div>
    <div ref="dragBox">
      ...
      >
        <span
          :class="dragTip.loadingIcon"
          :style="{
            transform: 'rotate(' + dragTip.iconRotate + 'deg)',
            opacity: dragTip.iconOpacity
          }"
        ></span>
      </div>
    </div>
    <div
      class="pullrefresh-draghelp"
      v-if="showHelpTag"
      @click="showHelpTag = false"
    >
...
export default {
  ...
  methods: {
    init() {
      ...
      this.$on("pullrefresh.finishLoad", this.finishLoad);
    ...
    bindEvents() {
      ...
      dragBox.addEventListener("touchstart", this.touchStartHandler, { passive: false });
      dragBox.addEventListener("touchmove", this.touchMoveHandler, { passive: false });
      dragBox.addEventListener("touchend", this.touchEndHandler, { passive: false });
