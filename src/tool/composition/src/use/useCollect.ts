      ...
      if (sort) {
        if (typeof sort !== "number") {
          ...
          sort = Array.from(el!.parentNode!.childNodes)
            .filter((childNode: any) => childNode.nodeName !== "#comment" && childNode.nodeName !== "#text" && (!childNode.style || childNode.style.display !== "none"))
            .indexOf(el);
        }
...
export function useCollect<Parent extends UseCollectComponent, Child extends UseCollectComponent>(
  ...
  const defaultParentName = counter();
  const decodeConfig = () => {
    const { parent } = config();
    const parentName = parent.name || defaultParentName;
    const provideString = `@@Collector_${parentName}`;
    return { parentName, provideString };
  };

  const parentCollector: ParentCollector<Child> = ((flag?: true) => {
    const { provideString } = decodeConfig();
