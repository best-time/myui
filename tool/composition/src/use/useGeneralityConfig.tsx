 ...
 * @date    2024.7.6 14:08
 */
export function useGeneralityConfig<Config extends { seq: number; code: string }>({ effects, getErrorMessage: _getErrorMessage, generalityConfigName }: { effects?: iEffects; getErrorMessage?: (code: string) => string; generalityConfigName?: string }) {
   ...
   * @date    2024.7.6 14:01
   */
  const registry = (seq: number, code: Config["code"], sourceConfig: Omit<Config, "code" | "seq"> | (() => Omit<Config, "code" | "seq">)) => {
    state.configMap = {
      ...state.configMap,
  ...
  const getByCodeForce = (code: Config["code"], getErrorMessage?: () => string): Config => {
    ...
    if (!ret) {
      const errorMessage = (getErrorMessage ? getErrorMessage() : _getErrorMessage?.(code)) || `${generalityConfigName}: Can't find config by code: ${code}!`;
