  ...
  methods: {
    ...
    getPanels() {
      return this.$children.filter(
        item => item.$options.name === "scrolltab-panel"
      );
    },
    bindEvent() {
    ...
    setDefault() {
      ...
      panels.forEach(panel => {
        if (panel.active) {
          ...
          ++num;
          if (num >= panels.length) this.activeIndex = panels[0]._uid;
        }
      });
    },
    moveHandler(uid) {
      ...
      const panels = this.getPanels();
      const itemOffsetTop = panels
        .filter(item => item._uid == uid)[0]
        .$el.getBoundingClientRect().top;

      this.scrollView.scrollTop =
        itemOffsetTop + this.scrollView.scrollTop - this.contentOffsetTop + 2;
