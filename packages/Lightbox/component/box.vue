<template>
  <div>
    ...
    <div class="lightbox" :style="bgStyleObject">
      <div class="lightbox-head" :class="show ? '' : 'lightbox-up-hide'">
      ...
      >
        ...
        <div class="lightbox-loading" v-show="showLoaingIcon">
          <svg
            width="100%"
            height="100%"
...
<script>
import Slider from "../../Slide/component/slider.vue";
import SliderItem from "../../Slide/component/slider-item.vue";
  ...
  methods: {
    ...
    touchEndHandler() {
      ...
      if (touches.isDarging) {
        ...
        if (
          timeDiff <= 200 ||
          Math.abs(touches.moveOffset) >= warpperSize * 0.2
        ) {
          this.close();
          return;
        }
    ...
    getTransform() {
      ...
      const x = -(
        body.offsetWidth / 2 -
        curImg.$el.offsetWidth / 2 -
        curImg.$el.getBoundingClientRect().left
      );
      const y = -(
        body.offsetHeight / 2 -
        curImg.$el.offsetHeight / 2 -
        curImg.$el.getBoundingClientRect().top
      );
    ...
    unBindEvent() {
      this.$refs.imgbox.removeEventListener(
        "touchstart",
