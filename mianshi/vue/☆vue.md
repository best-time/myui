...
其实，那是因为恰巧 computed 用到的字段，也被别的 computed 或者 render 使用了， 
因此当依赖项发生修改时，触发了render的重新执行，进而调用了返回的函数，实现了刷新。

computed 返回函数的话， 将没有办法建立起与依赖数据的关系，依赖数据修改后，也就无法刷新 computed 的值。
